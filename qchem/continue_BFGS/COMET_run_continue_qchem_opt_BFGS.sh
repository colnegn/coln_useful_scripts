...
if [ $MO_initrestart_bool != 1 ]; then
  ...
  sed -i 's/scf_guess/!scf_guess/' "$initinput"
fi

## do we already have HESS (BFGS guess)?
if [ $HESS_initrestart_bool != 1 ]; then
  ## don't have orbitals or Hessian to read:
  sed -i 's/GEOM_OPT_HESSIAN/!GEOM_OPT_HESSIAN/' "$initinput"
fi
...
while [ $jobscomplete != 1 ]; do
  ...
  if [ -f "$d/STDOUT" ]; then
    ...
    #!# check if we DO NOT have MO guesses:
    if ! [ -f "$d/$scratchdir/53.0" ]; then
    ...
    #!# check if there is a HESS (BFGS guess) file:
